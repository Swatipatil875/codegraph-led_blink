 ...
 *   codegraph explore <query>   Explore symbols + call paths (MCP parity)
 *   codegraph chat [path]       Terminal assistant (CodeGraph-only answers)
 ...
 * codegraph chat [path]
 *
 * Interactive terminal assistant for an indexed project. The LLM answers ONLY
 * by calling CodeGraph tools (explore, search, node, callers, callees, impact,
 * files, status) — no hardcoded project logic. Supports follow-up questions
 * via conversation history.
 *
 * Requires an OpenAI-compatible LLM endpoint (same config as `codegraph offload`).
 * Auto-initializes and indexes the project when --init is passed or when the
 * path has no .codegraph/ yet and --no-init was not set.
 */
program
  .command('chat [path]')
  .description('Terminal code assistant — answers from CodeGraph (local mode by default; use --llm for external LLM)')
  .option('--init', 'Initialize and index the project if not already indexed')
  .option('--no-init', 'Refuse to start when the project is not indexed (do not auto-init)')
  .option('--llm', 'Use an external LLM with tool calling (requires codegraph offload endpoint)')
  .option('-q, --query <text>', 'Run a single non-interactive query and exit')
  .option('--verbose-source', 'Show full CodeGraph source dumps instead of concise answers')
  .option('--quiet-tools', 'Hide CodeGraph tool-call progress on stderr')
  .action(async (
    pathArg: string | undefined,
    options: { init?: boolean; noInit?: boolean; llm?: boolean; query?: string; quietTools?: boolean; verboseSource?: boolean },
  ) => {
    const projectPath = path.resolve(pathArg || process.cwd());

    try {
      const { default: CodeGraph } = await loadCodeGraph();
      const shouldAutoInit = !options.noInit && (options.init || !isInitialized(projectPath));

      if (!isInitialized(projectPath)) {
        if (!shouldAutoInit) {
          error(`CodeGraph is not initialized in ${projectPath}. Run: codegraph init "${projectPath}" or use --init`);
          process.exit(1);
        }

        const unsafe = unsafeIndexRootReason(projectPath);
        if (unsafe) {
          error(`Refusing to index ${projectPath} — it looks like ${unsafe}. Pass --force on codegraph init if intentional.`);
          process.exit(1);
        }

        info(`Initializing and indexing ${projectPath}…`);
        const initCg = await CodeGraph.init(projectPath, { index: false });
        process.stdout.write(`${colors.dim}${getGlyphs().rail}${colors.reset}\n`);
        const progress = createShimmerProgress();
        const indexResult = await initCg.indexAll({ onProgress: progress.onProgress });
        await progress.stop();
        info(`Indexed ${indexResult.filesIndexed} files, ${indexResult.nodesCreated} symbols.`);
        initCg.destroy();
      }
      ...
      const { runRepl, runOnceQuery } = await import('../chat');
      const chatMode = options.llm ? 'llm' as const : 'local' as const;

      if (options.query) {
        const answer = await runOnceQuery(options.query, {
          projectPath,
          cg,
          mode: chatMode,
          concise: !options.verboseSource,
          verbose: !options.quietTools,
        });
        console.log(answer);
        cg.destroy();
        return;
      ...
      if (!process.stdin.isTTY) {
        error('Interactive chat requires a TTY. Use --query for non-interactive mode.');
        cg.destroy();
        process.exit(1);
      }
